...
namespace graphTheoryTools
{
    ...
    // Computes the Adjacency Matrix based on the Euclidean distance and neighborRadius
    Eigen::MatrixXd computeAdjacencyMatrix(std::unordered_map<int, std::vector<double>> states)
    {
    ...
    Eigen::MatrixXd computeDegreeMatrix(std::unordered_map<int, std::vector<double>> states)
    {
        Eigen::MatrixXd A = computeAdjacencyMatrix(states);
    ...
    Eigen::MatrixXd computeLaplacianMatrix(std::unordered_map<int, std::vector<double>> states)
    {
        Eigen::MatrixXd A = computeAdjacencyMatrix(states);
        Eigen::MatrixXd D = computeDegreeMatrix(states);
